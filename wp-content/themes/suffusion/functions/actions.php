...
/*	else if ($page_layout == 'no' || $page_layout == '1l' || $page_layout == '2l') {
		$ret = 0;
	}
	else if ($page_layout == '1r' || $page_layout == '1l1r') {
		$ret = 1;
	}
	else if ($page_layout == '2r') {
		$ret = 2;
	}*/
...
function suffusion_get_author_information() {
	...
	$ret .= "</p><!-- /.author-bio -->\n";
	if (is_author()) {
		$ret .= suffusion_get_author_profile_links($id);
	}
...
function suffusion_set_title() {
	...
	if ($suf_seo_enabled != 'enabled') {
		...
		// BP 1.5 makes bp_get_page_title deprecated

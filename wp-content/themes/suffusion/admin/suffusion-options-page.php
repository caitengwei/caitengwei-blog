...
function suffusion_admin_help_pages() {
	if (isset($_REQUEST['page']) && 'suffusion-options-manager' == $_REQUEST['page']) {
		...
		// Probably WP 3.1.x or 3.2.x, so we will use add_contextual_help
		else {
			$help_tabs = array(
				'theme-options-intro.php' => 'intro-pages',
				'theme-options-theme-skinning.php' => 'skinning',
				'theme-options-layouts.php' => 'layouts',
				'theme-options-typography.php' => 'typography',
				'theme-options-visual-effects.php' => 'visual-effects',
				'theme-options-sidebars-and-widgets.php' => 'sidebar-setup',
				'theme-options-blog-features.php' => 'blog-features',
				'theme-options-templates.php' => 'templates',
				'theme-options-custom-types.php' => 'custom-types',
			);
			if (isset($_REQUEST['tab']) && array_key_exists($_REQUEST['tab'], $help_tabs)) {
				$tab = $_REQUEST['tab'];
			}
			else {
			...
			$tab = $help_tabs[$tab];
			if (array_key_exists($tab, $suffusion_help_texts)) {
				$help = $suffusion_help_texts[$tab];
				add_contextual_help('appearance_page_suffusion-options-manager', "<h3>{$help['name']}</h3>".$help['help']);
			}
		}
	}
}
...
function suffusion_admin_script_loader() {
	...
	else if ($wp_version >= 3.2 && $wp_version < 3.3) {
